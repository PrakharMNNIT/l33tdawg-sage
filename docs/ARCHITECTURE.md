...
3. **Claimed/read** — `POST /v1/messages/receive` claims one ordered batch using a replayable receive token, recording the caller's opaque `claimant_session_id` as the owner of that claim. Concurrent runtimes sharing one signed agent identity therefore do not silently take each other's work: a second runtime must take ownership explicitly via `PUT /v1/messages/{id}/handoff`, a compare-and-swap on the current session, and a stale former owner is refused if it replies after ownership moved. Exact-recipient read acknowledgement is separate evidence and never means comprehension.
