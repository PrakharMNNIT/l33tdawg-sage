...
**Status (2026-07):** **v11.13.4 is the current release.** Governed app-v21 makes challenge resolution corroboration-aware: a fresh round snapshots the eligible modify holders plus current read-authorized canonical corroborators and requires one more distinct challenger than canonical supporters, while zero-corroborator noise still resolves on one challenge. A modify holder must open the dispute; snapshotted corroborators can then reverse their support by endorsing it. Distinct corroborator and lifetime challenger counts are now exposed through REST, federation, MCP, and the Python SDK. Authorized state sync supports exact-version-pinned app-v20 and app-v21 images. The complete CI/security/fault matrix remains a mandatory publication invariant, and the native-shell productization bridge remains targeted at v11.11–v11.14.
