<!-- Reference index reconciled for SAGE v11.18.25. Core REST, MCP, concepts, Python SDK, federation/brain graph, reranker, and environment references are current-facing for v11. -->
...
|----------|-----------------|
| [`rest-api.md`](rest-api.md) | HTTP endpoints: method, path, request/response fields, auth, clearance, and examples. |
...
| [`concepts/message-wake-bus.md`](concepts/message-wake-bus.md) | Durable payload-free local inbox wake-up: exact-recipient monotonic sequence allocation, signed `GET /v1/messages/wake` catch-up/SSE, `after_seq` / `Last-Event-ID`, single-consumer leases, crash/reconnect behavior, and why a wake is never delivery, read, presence, claim, or message content. |
...
| Read the reply a recipient returned for a message **you** sent | [`mcp-tools.md`](mcp-tools.md) — `sage_inbox.reply_items` for the normal one-call poll (v11.18.4+) or `sage_message_replies` for explicit backward paging; [`concepts/message-reply-lifecycle.md`](concepts/message-reply-lifecycle.md). `sage_message_status` remains deliberately payload-free. |
| Wake an exact local supervisor when canonical inbox work is durably inserted | [`rest-api.md`](rest-api.md) — signed `GET /v1/messages/wake`; [`concepts/message-wake-bus.md`](concepts/message-wake-bus.md) |
...
## Related docs (reconciled through v11.18.25)
