<!-- Reconciled through SAGE v11.19.15. Cite file:line when behavior is non-obvious. -->
...
`index_status` is present on an **EMPTY, domain-scoped semantic recall**. `"complete"` and `"incomplete"` are restricted to the exact unnarrowed local `status_filter: "committed"` contract: no provider, tags, minimum confidence, cursor, text arm, or federation request. `"complete"` means every committed/challenged row in that local domain is reachable in the active vector space, so the empty result is genuine local absence. `"incomplete"` means a committed/challenged row has no vector or belongs to a foreign vector space; empty is **not** proof of absence (re-embed the domain, or query the matching space). `"unavailable"` is still emitted for other empty domain-scoped semantic recalls and makes no completeness claim—for example when the query is narrowed or federated, the caller is not proven to see every record, the complete canonical SQL-to-ledger projection audit is not exact, there is no exact active vector space, or the live domain exceeds the bounded scan. The enum is never a count or list and is absent whenever any result is returned. Search/hybrid endpoints do not emit it.
...
**CEREBRUM orchestration (v11.3; app-v26 CAS; v11.17.15 governed bulk retry):** The dashboard drives this whole agent-to-agent transfer from the Search page via `POST /v1/dashboard/network/reassign-domain-ownership`, commit-confirmed in strict order: `gov_propose(domain_reassign)` -> the sole validator's accept vote drives the proposal to `Executed` in-band -> this `DomainReassign` atomically flips the owner, records ownership history, purges unrelated grants, applies any shared marker, and consumes the proposal. The new owner's access follows directly from canonical ownership, so no target-key lookup or self-grant is required. At app-v26 the dashboard reads the canonical current owner, includes it as `expected_owner_id` in both the approved proposal payload and execution transaction, and consensus compares it immediately before transfer. A concurrent handover therefore fails instead of applying a stale operator confirmation. The server returns `{status:"ok", already_owned:true}` when a retry targets the owner that already controls the domain. Internally generated app-v20 governance proofs first use the fresher of host wall time and the latest committed CometBFT time. If an idle-chain FinalizeBlock rejects that proof as more than five minutes ahead, the rejected transaction mutates no state but commits a new clock; the server re-signs the exact request once at that committed time. The ±5-minute consensus window is unchanged. For an explicitly confirmed multi-domain transfer, CEREBRUM retries only the exact proposer-cooldown rejection, displays current/required block progress, and stops on every other error. This is necessary because personal chains intentionally emit no empty heartbeat blocks; each rejected proposal advances the existing 50-block cooldown without weakening it. The optional trailing wire field is admitted only from H+1; activation height H retains the historical encoding. It requires a single-validator node; a multi-validator chain returns HTTP 409 because the other validators must vote on the proposal. This is off-consensus orchestration only - each underlying step remains the same on-chain transaction, and memory authorship (`submitting_agent`) is never rewritten (`web/reassign_handler.go`; `web/rbac_signing.go`; `web/static/js/governance-retry.js`; `internal/abci/app.go`).
...
Provider-addressed compatibility work is always atomically session-bound; an
omitted session uses the explicit `legacy` fence.

**Response** (HTTP 200): `{"pipe_id": "...", "status": "claimed"}` plus
`claimant_session_id` when a provider-addressed compatibility claim was bound.
HTTP 409 if already claimed.
...
`result` is capped at 256 KiB (`MaxPipeContentBytes`, `store.go:775`); an over-cap submission is rejected **HTTP 413**, enforced both at the handler and at the `CompletePipeline` store chokepoint (`sqlite.go:6741`, mapping `ErrPipeResultTooLarge` at `:6743-6744`).

**Response** (HTTP 200):
`{"status":"completed","journal_id":"<memory_id or empty>","journaled":true|false}`.
Provider-addressed compatibility completion additionally returns
`idempotent_replay:true|false`. Its completion CAS, sealed result fingerprint,
and any pre-app-v23 compatibility journal commit atomically, so concurrent
identical retries return the winner's authoritative `journal_id` and publish no
...
For provider-addressed legacy work, MCP reaches this route only after
`POST /v1/messages/{message_id}/reply` returns the exact canonical typed
compatibility signal above. It carries the same `claimant_session_id` used by
the canonical attempt. Generic conflicts and typed canonical 404 responses do
not authorize this fallback. The claim session, exact persisted claimant, TTL,
completion CAS, and sealed result fingerprint are checked transactionally. An
identical lost-response retry returns HTTP 200 with `idempotent_replay:true`
without duplicating its journal; a different result returns HTTP 409 `Reply
conflict`.
