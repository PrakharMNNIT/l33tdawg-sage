<!-- Reconciled through SAGE v11.18.26. Cite file:line when behavior is non-obvious. -->
...
`index_status` is present **only on an EMPTY, domain-scoped recall** of the default (`status_filter: "committed"`) contract, so a caller can tell genuine absence from an indexing gap. `"complete"` — every committed/challenged row in the domain is reachable in the active vector space, so the empty result is real absence. `"incomplete"` — the domain holds committed/challenged rows that no vector, or a foreign vector space, keeps recall from returning; empty is **not** proof of absence (re-embed the domain, or query the space those rows live in). `"unavailable"` — the verdict could not be proven leak-free and no claim is made: the caller is not proven to see every record in the domain, there is no exact active vector space, a non-default `status_filter` was used, or the domain is too large to settle within the bounded scan. The field is emitted **only to a caller proven to see every record in the domain**, and is computed with no per-record authorization, so it can never disclose the existence or number of records hidden from that caller. It is a three-value enum — never a count or a list — and is absent whenever any result is returned.
...
`result` is capped at 256 KiB (`MaxPipeContentBytes`, `store.go:513`); an over-cap submission is rejected **HTTP 413**, enforced both at the handler and at the `CompletePipeline` store chokepoint (`sqlite.go:6748`, mapping `ErrPipeResultTooLarge`).
