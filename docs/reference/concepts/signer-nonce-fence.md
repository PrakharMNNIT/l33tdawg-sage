...
| Re-submission refused with **CheckTx code 4** (nonce gate) | **yes** | The signer's committed nonce is monotone non-decreasing **on the chain**, so this refusal cannot un-happen there: the bytes can never commit *again*. Two caveats the code itself writes down: (a) code 4 proves supersession **or self-commit** — the gate is `nonce <= committed`, so a transaction that itself committed answers code 4 exactly like one overtaken by a higher nonce, and without the tx index the two are indistinguishable (see triage step 2); (b) a node-level rollback (snapshot restore, state-sync rewind) is the one event that can lower the committed nonce — accepted, because a restore invalidates the fence-holding process anyway. |
