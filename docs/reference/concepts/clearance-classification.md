...
`POST /v1/memory/submit` → `handleSubmitMemory` (`api/rest/memory_handler.go:1176`, classification read at `:1212`):
...
`GET /v1/memory/{memory_id}` (`handleGetMemory`, `memory_handler.go:3294+`) also reads classification from BadgerDB and returns it in the response body. The same `if memClass > 0` gate logic applies to single-record fetches.
