...
| `SAGE_CLAUDE_CHANNEL` | Controls the experimental Claude wake channel. It is off for every host unless explicitly enabled with `1`/`true`/`yes`. When on, `sage-gui mcp` subscribes to this agent's signed `/v1/messages/wake` stream and emits a `notifications/claude/channel` JSON-RPC notification. The subscription owns the one exact-agent wake lease, so enable it only when the attached host consumes that custom notification; short-lived secondary processes should use the lease-free wake-state route instead. Payload-free: the host learns only a durable wake cursor, never message content or sender. An unrecognized value warns and stays off. Failure to arm is non-fatal and leaves an ordinary MCP session. | off | sage-gui MCP (stdio) | `claudeChannelEnabled` (`mcp.go:241`), `EnableRESTClaudeChannel` (`internal/mcp/claude_wake_source.go:85`) |
