...
| `SAGE_STOP_NUDGE` | Opt-in end-of-turn check. When on, the generated Claude Code Stop hook asks `sage-gui hook stop-check` whether durable unclaimed inbox work is pending and, if so, declines the stop once so the agent handles it in-session rather than going idle on it. Payload-free: the nudge is derived from a count and carries no message identity or content. Bounded twice — never blocks when `stop_hook_active` is set, and never re-blocks the same session until the unread count grows — and fails open on every error. | off | sage-gui hook, Claude Code Stop/SubagentStop | `cmd/sage-gui/hook.go` (`stopNudgeEnabled`), `cmd/sage-gui/mcp.go` (`sageStopScript`) |
