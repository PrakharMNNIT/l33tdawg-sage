...
| `UserPromptSubmit` | `sage-user-prompt.sh` | passive pointer + nudge | In `full` and `bookend`, calls `sage-gui hook inbox-status` to surface payload-free exact-agent unfinished-work state without claiming work. It combines the unclaimed count with the lease-free wake snapshot, so claimed-but-unfinished work cannot look clear. `full` also reminds the agent to call `sage_turn`; `bookend` suppresses only that memory nudge. Probe failures are reported as unavailable, never as zero. |
| `Stop` | `sage-stop.sh` | turn-boundary nudge | Calls `sage-gui hook stop-check`. It defaults on for Claude Code, Codex, and legacy installed Stop hooks whose provider label is absent; disable it with `SAGE_STOP_NUDGE=0`. When the durable wake sequence advances while exact-recipient work remains unfinished (claimable or already claimed), it declines the stop once so the agent handles or explicitly declines that generation before going idle. The check reads only the payload-free lease-free wake snapshot, never blocks twice in a row (`stop_hook_active`), never re-nudges the same session for work it already declined, and fails open—including when its one-shot cursor cannot be persisted. |
