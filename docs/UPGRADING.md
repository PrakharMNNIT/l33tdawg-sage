...
| v11.18.15 | Unfinished-message wake backfill for claimed-only upgrades, atomic exact-local legacy-pipe admission, explicit opt-in for the experimental Claude notification adapter, deterministic pending-memory tiebreaks, and anchor-aware citation repair with pinned parser debt; no new app version; app-v26 remains the ceiling |
