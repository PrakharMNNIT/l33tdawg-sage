...
function RecallSettings() {
    ...
    useEffect(() => {
        ...
        fetchReranker().then(c => setRrOn(!!c.enabled)).catch(() => {});
    }, []);
    ...
    return html`
        ...
        <div class="settings-row" style="align-items:flex-start;margin-top:8px">
            <div style="flex:1">
                ...
                <div style="font-size:11px;color:var(--text-dim);margin-top:2px;line-height:1.5;">
                    Only memories at or above this consensus confidence are returned. Confidence is earned
                    (validator quorum, corroboration) and decays with age, so this is a freshness + trust
                    filter: high = strict and reliable, lower = broader but noisier recall.
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:10px;min-width:180px">
...
// RerankerSetupModal - the zero-terminal "set it up for me" flow. One consent
// click, then SAGE does everything itself: downloads a pinned llama.cpp
// release build (Ollama has no rerank endpoint), downloads the pinned
// bge-reranker-v2-m3 GGUF, verifies both by sha256, then spawns and manages
// the sidecar. Manual instructions appear only when the platform has no
// pinned build or a download fails.
function RerankerSetupModal({ onClose, onDone }) {
    const [st, setSt] = useState(null);
    const [phase, setPhase] = useState('check'); // check | unavailable | manual | consent | running | done
    const [err, setErr] = useState(null);
    const [stepState, setStepState] = useState({});
    const [prog, setProg] = useState(null); // { step, done, total }
    ...
    const streamStep = async (fetchFn, step) => {
        setStepState(p => ({ ...p, [step]: 'active' }));
        const res = await fetchFn();
        if (!res.ok) throw new Error('HTTP ' + res.status);
        let failed = null;
        await readLines(res, (k, v) => {
            if (k === 'progress') { const parts = v.split(' '); setProg({ step, done: Number(parts[0]) || 0, total: Number(parts[1]) || 0 }); }
            if (k === 'error') failed = v;
        });
        setProg(null);
        if (failed) throw new Error(failed);
        setStepState(p => ({ ...p, [step]: 'done' }));
    };
    ...
    const runAll = async () => {
        ...
        const s0 = st || {};
        const needEngine = !(s0.binary_found || s0.engine_installed);
        setStepState({ engine: needEngine ? 'pending' : 'done', model: s0.model_ready ? 'done' : 'pending', start: 'pending' });
        try {
            if (needEngine) await streamStep(rerankerSetupInstallEngine, 'engine');
            if (!s0.model_ready) await streamStep(rerankerSetupDownload, 'model');
            setStepState(p => ({ ...p, start: 'active' }));
            await rerankerSetupStart();
            setStepState(p => ({ ...p, start: 'done' }));
            await refresh();
            setPhase('done');
            if (onDone) onDone();
        } catch (e) {
    ...
    const recheckManual = async () => { const s = await refresh(); if (s && s.binary_found) setPhase('consent'); };
    const mb = (n) => ((n || 0) / (1024 * 1024)).toFixed(0);

    const needEngine = st && !(st.binary_found || st.engine_installed);
    const needModel = st && !st.model_ready;
    const totalMB = st ? Number(mb((needEngine ? (st.engine_bytes || 0) : 0) + (needModel ? (st.model_bytes || 0) : 0))) : 0;

    const stepRow = (key, label) => {
        const s = stepState[key] || 'pending';
        const active = s === 'active';
        ...
        return html`
            <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
                ...
                <div style="flex:1;">
                    ...
                    ${pct !== null && html`
                        <div style="height:6px;background:var(--bg-elev);border-radius:3px;overflow:hidden;margin-top:5px;">
                            <div style="height:100%;width:${pct}%;background:var(--primary);transition:width 0.3s;"></div>
                        </div>
                        <div style="font-size:11px;color:var(--text-muted);margin-top:3px;">${mb(prog.done)} / ${mb(prog.total)} MB (${pct}%)</div>
                    `}
                    ${active && pct === null && html`<div style="font-size:11px;color:var(--text-muted);margin-top:3px;">Working\u2026</div>`}
                </div>
            </div>`;
    };
    ...
    return html`
        <div class="wizard-overlay" onClick=${e => { if (e.target === e.currentTarget) requestClose(); }}>
            <div class="wizard-modal" ref=${dialogRef} tabIndex="-1" role="dialog" aria-modal="true" aria-labelledby="reranker-setup-title" style="max-width:540px;">
                ...
                <div class="wizard-body" style="padding:20px;line-height:1.55;">
                    ...
                    ${phase === 'consent' && html`
                        ...
                        <ul style="padding-left:20px;color:var(--text-dim);font-size:13px;">
                            ...
                            <li>Start it locally and switch recall over - SAGE manages the process from then on</li>
                        </ul>
                        ${err && html`<div class="import-error" style="margin:10px 0;">${err}</div>`}
                        <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px;">
                            <button class="btn btn-primary" onClick=${runAll}>${err ? 'Retry' : (totalMB > 0 ? `Set it all up (${totalMB} MB download)` : 'Set it all up')}</button>
                        </div>
                    `}
                    ${phase === 'running' && html`
                        <div>
                            ${stepRow('engine', 'Reranker engine (llama.cpp)')}
                            ${stepRow('model', st ? st.model_name : 'Reranker model')}
                            ${stepRow('start', 'Start + switch recall over')}
                        </div>
                        <p style="color:var(--text-muted);font-size:12px;margin-top:12px;">Leave this open until it finishes - a few minutes on a typical connection.</p>
                    `}
                    ${phase === 'manual' && html`
                        <p style="margin-top:0;">There's no pinned llama.cpp build for this platform, so this one step is manual - install it, then come back:</p>
                        ...
                        <p style="color:var(--text-dim);font-size:12px;">No package manager? Grab a release from github.com/ggml-org/llama.cpp/releases and put llama-server on your PATH.</p>
                        <div style="display:flex;justify-content:flex-end;margin-top:16px;">
                            <button class="btn btn-primary" onClick=${recheckManual}>I've installed it - continue</button>
                        </div>
                    `}
                    ${phase === 'done' && html`
                ...
                <div class="wizard-footer">
                    <button class="btn" onClick=${() => { if (!busy) onClose(); }} disabled=${busy}>${phase === 'done' ? 'Done' : 'Close'}</button>
                </div>
            </div>
        </div>
    `;
...
function NetworkPage({ sse, accessMode = false }) {
    ...
    return html`
        <div class="network-page fade-in ${accessMode ? 'access-mode' : ''}">
            ...
            ${showChatGPTWizard && html`<${ChatGPTTunnelWizard} onClose=${() => setShowChatGPTWizard(false)} />`}
            ...
            ${showConnectTool && html`<${ConnectToolModal} agents=${agents}
                onOpenChatGPT=${() => { setShowConnectTool(false); setShowChatGPTWizard(true); }}
...
function ChatGPTTunnelWizard({ onClose }) {
    ...
    const initCommand = `tunnel-client init \\
        ...
        <div class="wizard-overlay" onClick=${e => { if (e.target === e.currentTarget) onClose(); }}>
            <div class="wizard-modal" ref=${dialogRef} tabIndex="-1" role="dialog" aria-modal="true" aria-labelledby="chatgpt-tunnel-title" style="max-width:720px;max-height:85vh;display:flex;flex-direction:column;">
                ...
                <div class="wizard-body" style="overflow-y:auto;flex:1;padding:20px;line-height:1.55;">
                    ...
                    <div class="wizard-field">
                        <label>OpenAI tunnel_id</label>
                        <input class="wizard-input" value=${tunnelId} onInput=${e => setTunnelId(e.target.value)}
                            placeholder="tunnel_0123456789abcdef0123456789abcdef" spellcheck="false" />
                    </div>
                    <div class="wizard-field">
                        <label>Local profile name</label>
...
function ChatGPTCopyField({ label, value, sensitive, multiline }) {
    ...
    return html`
        <div style="display:grid;grid-template-columns:180px 1fr auto;gap:8px;align-items:${multiline ? 'start' : 'center'};margin-bottom:8px;">
            <div style="font-size:12px;color:var(--text-dim);">${label}</div>
            ${multiline
                ? html`<pre style="background:var(--bg-elev);padding:8px;border-radius:3px;font-size:12px;white-space:pre-wrap;word-break:break-word;margin:0;color:var(--text);">${display}</pre>`
                : html`<code style="background:var(--bg-elev);padding:6px 8px;border-radius:3px;font-size:12px;word-break:break-all;">${display}</code>`}
        ...
        <div class="wizard-overlay" onClick=${e => { if (e.target === e.currentTarget) onClose(); }}>
            <div class="wizard-modal" style="max-width:620px;max-height:85vh;display:flex;flex-direction:column;">
                ...
                <div class="wizard-body" style="overflow-y:auto;flex:1;padding:20px;line-height:1.55;">
                    ...
                    <p>These clients can also take a bearer token directly — <strong>no tunnel, no public URL, no external account needed</strong>. They run on the same machine as SAGE and talk to <code>https://localhost:8443</code>.</p>
...
// RemotePasteBlocks — renders BOTH client families' paste blocks for an
// operator-managed HTTPS endpoint or a direct LAN URL.
function RemotePasteBlocks({ url, token, selfSigned }) {
    ...
    return html`
        ...
        ${selfSigned && html`<div class="warning-banner" style="margin-top:8px;">Stock Cursor/Cline will reject a self-signed certificate — use a trusted HTTPS endpoint for these.</div>`}
...
// REMOTE_PROVIDERS — tools running on ANOTHER computer that reach this
...
//   'openai-tunnel' — ChatGPT via OpenAI Secure MCP Tunnel; routed to the dedicated wizard
...
// this node via a direct LAN bind or an operator-managed reachable URL, so we first ask the node
...
// URL, or (ChatGPT) hand off to the OpenAI tunnel wizard. When there's NO remote
// path we say so plainly.
                    ...
                    ${view === 'branch' && html`
                        ...
                        <div class="connect-cards" style="grid-template-columns:1fr;margin-top:16px;">
                            ...
                            <div class="connect-card" style="text-align:left;" role="button" tabIndex="0"
                                ...
                                <div class="connect-card-icon">\u{1F310}</div>
                                <h4>On another computer (AI tool)</h4>
                                <p>Cursor, Cline, Windsurf, or Claude Desktop on a different machine using a LAN/VPN or reachable HTTPS endpoint.</p>
                            </div>
                            <div class="connect-card" style="text-align:left;" role="button" tabIndex="0"
...
function OverviewPage({ sse }) {
    ...
    const [showEmbedSetup, setShowEmbedSetup] = useState(false);  // engine CTAs: setup runs right here,
    const [showRerankSetup, setShowRerankSetup] = useState(false); // not buried three clicks into Settings
    ...
    return html`<div class="settings-page">
        ...
        ${sectionCard('Embeddings',
            ...
            ${health && (embFallback || !rrOn) ? html`
                <div style="display:flex;gap:10px;margin-top:12px;flex-wrap:wrap;align-items:center;">
                    ${embFallback ? html`
                        <button class="btn btn-primary" onClick=${() => setShowEmbedSetup(true)}>Turn on smart memory →</button>
            ...
            ` : ''}`)}
        ${showEmbedSetup && html`<${EmbeddingsSetupModal} onClose=${() => { setShowEmbedSetup(false); fetchHealth().then(setHealth).catch(() => {}); }} onDone=${() => fetchHealth().then(setHealth).catch(() => {})} />`}
        ${showRerankSetup && html`<${RerankerSetupModal} onClose=${() => { setShowRerankSetup(false); fetchHealth().then(setHealth).catch(() => {}); }} onDone=${() => fetchHealth().then(setHealth).catch(() => {})} />`}
    </div>`;
}
...
function SharingSyncGroupsPanel({ connections = [], reachability = {} }) {
	...
	const dissolveGroup = async groupId => {
		setBusy(groupId + ':dissolve'); setError('');
		try {
			await fedGroupDissolve(groupId);
			showToast('Sharing group deleted; trusted connections are unchanged', 'success');
			await loadGroups();
			requestAnimationFrame(() => document.getElementById('sharing-sync-title')?.focus());
		} catch (e) {
			await loadGroups();
			const count = Number(e.data && e.data.removed_members || 0);
			const message = e.data && e.data.status === 'dissolving'
				? (count > 0
					? `Closing paused after ${count} ${count === 1 ? 'member was' : 'members were'} updated. Group sharing is already stopped. Retry to finish.`
					: 'Closing is still paused. Group sharing remains stopped. Retry to finish.')
				: String(e.message || e);
			setError(message); showToast(message, 'error');
		}
		setBusy('');
    ...
    return html`<section class="fed-groups" aria-labelledby="sharing-sync-title">
        ...
        ${groups && groups.map((group, groupIndex) => {
			...
			const groupClosing = group.lifecycle_state === 'dissolving';
			...
			const groupConnected = !groupClosing && (remoteMembers.length === 0 || activeRemoteMembers.length > 0);
			const groupConnectionSummary = groupClosing
				? 'closing — retry needed'
				: !groupReady
                ? 'setup in progress'
            ...
            return html`<article class="fed-group-card" key=${groupId}>
                ...
                ${openGroup === groupId && html`<div id=${panelId} class="fed-group-detail">
                    ...
                    ${group.is_controller && html`<section class="fed-perm-danger">
						...
						<p>Remove every member and stop sharing through this group. This does not end any trusted connection. Offline SAGEs are updated when they next connect.</p>
                        <button type="button" class="btn btn-danger" disabled=${busy !== ''} onClick=${async () => {
							if (!groupClosing && !await showConfirmation('Delete ' + displayName + '? Every member is removed from this sharing group and sharing through it stops. This does not end their trusted connection to this SAGE. Each SAGE keeps copies it already has, and offline SAGEs are updated when they next connect.', { title: 'Delete sharing group?', confirmLabel: 'Delete group', tone: 'danger' })) return;
							await dissolveGroup(groupId);
						}}>${groupClosing ? 'Retry closing group' : 'Delete sharing group'}</button>
                    </section>`}
...
function FederationPage() {
    ...
    const load = async () => {
        ...
        try {
			...
			// A fresh active pairing is a new trust generation. Clear every local
			// history dismissal for that peer so a later expiry/revocation cannot be
			// hidden by a key left over from an older connection that lacked ended_at.
			const activeChains = new Set(next.filter(c => c.status === 'active' && !c.expired).map(c => c.remote_chain_id));
			if (activeChains.size > 0) setHiddenPastConnections(current => {
				const visible = current.filter(key => !Array.from(activeChains).some(chain => key.startsWith(chain + ':')));
				if (visible.length !== current.length) {
					try { localStorage.setItem('sage-fed-hidden-past-connections', JSON.stringify(visible)); } catch (e) {}
					return visible;
				}
				return current;
			});
    ...
    const pastConnectionKey = connection => `${connection.remote_chain_id}:${connection.ended_at || connection.status || 'ended'}`;
    const hidePastConnection = connection => {
        const key = pastConnectionKey(connection);
        setHiddenPastConnections(current => {
            const next = current.includes(key) ? current : [...current, key];
            try { localStorage.setItem('sage-fed-hidden-past-connections', JSON.stringify(next)); } catch (e) {}
            return next;
        });
    };
    const showHiddenPastConnections = () => {
        try { localStorage.removeItem('sage-fed-hidden-past-connections'); } catch (e) {}
        setHiddenPastConnections([]);
    };
            ...
            ${(fedOn || (conns && conns.length > 0)) && html`<div class="fed-conns">
                ...
                ${pastConns.length > 0 && html`<div class="fed-past">
                    <button class="fed-past-toggle" onClick=${() => setShowPast(!showPast)} aria-expanded=${showPast} aria-controls="fed-past-connections">
                        <span>${showPast ? '▾' : '▸'}</span> Previous connections (${visiblePastConns.length})
                    </button>
                    ${showPast && html`<div class="fed-past-list" id="fed-past-connections">
                        ${visiblePastConns.map(c => html`<div class="fed-conn-row fed-conn-past" key=${pastConnectionKey(c)}>
                            <span class="fed-conn-status off"></span>
                            <span class="fed-conn-name">${c.peer_name || 'Other SAGE'}</span>
                            <span class="fed-conn-meta muted">${c.expired ? 'expired' : (c.ended_by === 'revoked_by_peer' ? 'ended by the other SAGE' : 'ended')}</span>
                            ...
                            <span class="fed-past-actions">
                                ${fedOn && html`<button class="btn" onClick=${() => beginGuestRejoin(c)}>Pair again…</button>`}
                                <button class="btn" onClick=${() => hidePastConnection(c)}>Hide from this list</button>
                            </span>
                        </div>`)}
                        ${visiblePastConns.length > 0 && html`<p class="muted">Pairing again requires a new connection code and approval from both people.</p>`}
                        ${hiddenPastCount > 0 && html`<button class="btn" onClick=${showHiddenPastConnections}>Show ${hiddenPastCount} hidden ${hiddenPastCount === 1 ? 'connection' : 'connections'}</button>`}
                    </div>`}
                </div>`}
            </div>`}
