...
export async function refreshTaskSnapshot({ fetcher, settlingIDs = new Set(), silent = false, setTasks, setDomains, setError }) {
  try {
    const data = await fetcher({ all: true, limit: 500 });
    ...
    if (!silent) setError(TASK_LOAD_ERROR);
    throw error;
  }
}
