...
//
// Callers build paths as `"/v1/dashboard/tasks?" + q.Encode()`, and
// url.Values{}.Encode() returns "" when every parameter is optional and none
// was set. So the client signed "/v1/dashboard/tasks?" while the verifier
// rebuilt "/v1/dashboard/tasks" from r.URL.Path plus an empty r.URL.RawQuery
...
func TestEmptyQueryDoesNotLeaveATrailingQuestionMarkInTheSignedPath(t *testing.T) {
	q := url.Values{}
	built := "/v1/dashboard/tasks?" + q.Encode()
	if built != "/v1/dashboard/tasks?" {
