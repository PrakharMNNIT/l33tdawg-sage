...
func TestReceiptV2ImportedClaimAndOutboxAreAtomicIdempotentAndRestartSafe(t *testing.T) {
	...
	replayed, err := s.RecordImportedFederatedReceipt(ctx, msg.PipeID, msg.ToAgent, event, "session-recipient")
	require.NoError(t, err)
	require.False(t, replayed)
	...
	history, err := s.GetInboxHistory(ctx, msg.ToAgent, "", 10)
	require.NoError(t, err)
	require.Len(t, history, 1)
	require.Equal(t, "session-recipient", history[0].ClaimedSessionID,
		"receipt-v2 claim and session fence must commit atomically")
	...
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, reopened.Close()) })
	replayed, err = reopened.RecordImportedFederatedReceipt(ctx, msg.PipeID, msg.ToAgent, event, "session-recipient")
	require.NoError(t, err)
	require.True(t, replayed)
	var count int
