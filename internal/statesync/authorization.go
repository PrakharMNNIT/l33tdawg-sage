...
const (
	...
	LatestSupportedAppVersion uint64 = 24
	cometNodeIDBytes                 = 20
...
func SupportsAppVersion(version uint64) bool {
	switch version {
	case RequiredAppVersion, 21, 22, 23, LatestSupportedAppVersion:
		return true
	default:
		return false
	}
...
func validateJoinAuthorization(config JoinAuthorizationConfig, now time.Time) (*joinAuthorization, error) {
	...
	if !SupportsAppVersion(config.AppVersion) {
		return nil, errors.New("state sync join authorization requires a supported app version (20, 21, 22, 23, or 24)")
	}
